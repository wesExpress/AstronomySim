...
vec3 calc_blackbody_light(blackbody bb, vec3 normal, vec3 frag_pos, vec3 view_dir, vec3 diffuse_color, vec3 specular_color, vec3 texture_color)
{
	...
	return (diffuse + specular) * texture_color * bb.brightness.x / 2048.0f;
}
...
void main()
{
	...
	{
		vec3 bb_color = calc_blackbody_light(blackbodies[j], norm_normal, vs_output.frag_pos, view_dir, texture_color.xyz, vs_output.obj_diffuse.xyz, vs_output.obj_specular.xyz);
