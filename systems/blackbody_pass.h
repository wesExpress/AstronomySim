...
static blackbody_instance bb_instances[BLACKBODY_MAX_MESHES][DM_MAX_INSTS] = { 0 };
static uint32_t           bb_insts_count[BLACKBODY_MAX_MESHES] = { 0 };

bool blackbody_pass(dm_entity* entities, uint32_t entity_count)
{
    dm_memzero(bb_insts_count, sizeof(uint32_t) * BLACKBODY_MAX_MESHES);
    ...
    for(uint32_t i=0; i<entity_count; i++)
    {
        ...
        blackbody_instance* inst = &bb_instances[mesh_handle][bb_insts_count[mesh_handle]++];
        ...
        inst->color = dm_vec4_set(color_r[entity], color_g[entity], color_b[entity], color_a[entity]);
    ...
    for(uint32_t i=0; i<bb_handles.num_meshes; i++)
    {
        ...
        uint32_t num = bb_insts_count[i];
        if(num==0) continue;
        
        dm_render_command_update_buffer(bb_handles.instb, &bb_instances[i], sizeof(blackbody_instance) * num, 0);
...
bool __blackbody_pass_init(float* positions, float* tex_coords, uint32_t num_vertices, uint32_t* indices, uint32_t num_indices, dm_render_handle* mesh_handles, uint32_t num_meshes)
{
    ...
    // resources
    if(!DM_CREATE_STATIC_INDEX_BUFFER(indices, uint32_t, num_indices, bb_handles.ib)) return false;
    if(!DM_CREATE_STATIC_VERTEX_BUFFER(vertices, blackbody_vertex, num_vertices, bb_handles.vb)) return false;
    if(!DM_CREATE_DYNAMIC_VERTEX_BUFFER(NULL, blackbody_instance, DM_MAX_INSTS, bb_handles.instb)) return false;
    
    dm_free(vertices);
    
#ifdef DM_OPENGL
    dm_render_handle vb_buffers[] = { bb_handles.vb, bb_handles.instb };
    if(!DM_RENDERER_CREATE_RENDERPASS("assets/shaders/blackbody_vertex.glsl", "assets/shaders/blackbody_pixel.glsl", vb_buffers, unis, attrib_descs, pipeline_desc, bb_handles.pass)) return false;
#else
    if(!DM_RENDERER_CREATE_RENDERPASS("assets/shaders/blackbody_vertex.fxc", "assets/shaders/blackbody_pixel.fxc", unis, attrib_descs, pipeline_desc, bb_handles.pass)) return false;
#endif
    ...
    DM_ECS_REGISTER_SYSTEM_EXCLUDES(DM_ECS_SYSTEM_TIMING_RENDER, blackbody_system_component_ids, blackbody_system_exclude_ids, blackbody_pass, render_system);
