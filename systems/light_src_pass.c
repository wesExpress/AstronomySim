...
bool light_src_pass_init(float* positions, float* tex_coords, uint32_t num_vertices, uint32_t* indices, uint32_t num_indices, dm_render_handle* mesh_handles, uint32_t num_meshes, dm_ecs_id id, view_camera* camera)
{
    ...
    // resources
    if(!dm_renderer_create_static_index_buffer(indices, sizeof(uint32_t) * num_indices, &light_handles.ib)) return false;
    
    if(!dm_renderer_create_static_vertex_buffer(vertices, sizeof(light_src_vertex) * num_vertices, sizeof(light_src_vertex), &light_handles.vb)) return false;
    if(!dm_renderer_create_dynamic_vertex_buffer(NULL, sizeof(light_src_instance) * DM_MAX_INSTS, sizeof(light_src_instance), &light_handles.instb)) return false;
